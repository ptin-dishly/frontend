...
export default function PublicMenuPage() {
  ...
  const listAllergenPool = useMemo(() => {
    return Array.from(
      new Map(
  ...
  const filteredMenus = useMemo(() => {
    if (excludedAllergenIds.length === 0) return menus;
  ...
  }, [menus, menusWithAllergens, excludedAllergenIds]);
    ...
    return (
      <div style={{ background: "#FFFBF5", minHeight: "100vh" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          {/* Allergen filter — list view */}
          {listAllergenPool.length > 0 && (
            <div style={{ padding: "10px 16px", borderBottom: "1px solid #F3E8D0", display: "flex", gap: 4, flexWrap: "wrap" }}>
              {listAllergenPool.map((a) => {
                const active = excludedAllergenIds.includes(a.id);
                return (
                  <button
                    key={a.id}
                    onClick={() => setExcludedAllergenIds((prev) => active ? prev.filter((id) => id !== a.id) : [...prev, a.id])}
                    title={a.nameEs}
                    style={{
