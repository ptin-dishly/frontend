    ...
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#F9FAFB" }}>
      ...
      <main style={{ flex: 1, padding: "40px 48px" }}>
        <div style={{ marginBottom: 30 }}>
          <h1 style={{ fontSize: 32, color: "#0F172A", margin: 0 }}>{t("ingredients.title")}</h1>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, gap: 16, flexWrap: "wrap" }}>
          <div style={{ maxWidth: 300, width: "100%" }}>
            <SearchBar value={globalSearch} onChange={setGlobalSearch} placeholder={t("ingredients.searchPlaceholder")} />
          </div>

          <div style={{ maxWidth: 200 }}>
            <SelectDropdown
              options={[{ label: t("ingredients.all"), value: "" }, ...categories]}
              value={categoryFilter}
              onChange={setCategoryFilter}
              placeholder={t("ingredients.category")}
            />
          </div>
          ...
          <div style={{ maxWidth: 200 }}>
            <SelectDropdown
              options={[
                { label: t("ingredients.allAllergens"), value: "" },
                ...allAllergens.map((allergen) => ({ label: allergen.nameEs, value: allergen.id })),
              ]}
            ...
            {filteredIngredients.length === 0 ? (
              ...
              filteredIngredients.map((ingredient) => (
                <div key={ingredient.id} style={{ position: "relative" }}>
                  <div style={{ backgroundColor: "white", borderRadius: 12, overflow: "hidden", border: "1px solid #E5E7EB", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", padding: 16, display: "flex", flexDirection: "column", gap: 12, minHeight: 280 }}>
                    {editingId === ingredient.id && editingData ? (
                      <>
                        <input
                          ...
                          style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #E5E7EB", fontSize: 13, fontFamily: "inherit" }}
                        />
                        <textarea
                          value={editingData.description || ""}
                          onChange={(e) => setEditingData({ ...editingData, description: e.target.value })}
                          placeholder={t("ingredients.descriptionPlaceholder")}
                          style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #E5E7EB", fontSize: 13, fontFamily: "inherit", minHeight: 60 }}
                        />
                        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
                          <input type="checkbox" checked={editingData.isActive || false} onChange={(e) => setEditingData({ ...editingData, isActive: e.target.checked })} />
                          {t("common.active")}
                        </label>
                        <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                          <button onClick={() => handleSave(ingredient.id)} disabled={savingId === ingredient.id} style={{ flex: 1, padding: "6px 12px", backgroundColor: "#22C55E", color: "white", border: "none", borderRadius: 6, cursor: savingId === ingredient.id ? "not-allowed" : "pointer", fontSize: 12, fontWeight: 600 }}>
                            {savingId === ingredient.id ? t("common.saving") : t("common.save")}
                          </button>
                          <button onClick={handleCancel} style={{ flex: 1, padding: "6px 12px", backgroundColor: "#E5E7EB", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>
                            {t("common.cancel")}
                          </button>
                        </div>
                      </>
                      ...
                      <>
                        <div>
                          <h3 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 600, color: "#0F172A" }}>{ingredient.name}</h3>
                          <p style={{ margin: 0, fontSize: 12, color: "#6B7280", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {ingredient.description || t("ingredients.noDescription")}
                          </p>
                        </div>
                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          {ingredientAllergens[ingredient.id] && ingredientAllergens[ingredient.id].length > 0 ? (
                            ...
                            <p style={{ margin: 0, fontSize: 11, color: "#9CA3AF" }}>{t("ingredients.noAllergens")}</p>
                          )}
                        </div>
